...
def add_source_table(doc):
    ...
    rows = [
        ...
        ("ChEMBL/PubChem", "Target-linked compounds and assay records", "1,021,439 records; 1,019,354 ChEMBL and 2,085 PubChem; 566 genes", "(9,10)"),
...
def build():
    ...
    p=add_para(doc,"Manuscript type: Web Server article | Draft date: 13 August 2026",style="Compact",align=WD_ALIGN_PARAGRAPH.CENTER)
    ...
    add_para(doc,"Table 1. Data sources and the audited database snapshot used for this manuscript (13 August 2026). Counts describe records stored in the local KinomeX instance and are not counts of unique biological observations across source databases.",style="Figure Caption")
    ...
    p.add_run(" and should be archived with a versioned DOI before submission. The manuscript snapshot was audited on 13 August 2026. Original records remain available from UniProt, KinHub/KinMap, STRING, RCSB PDB, AlphaFold DB, ChEMBL, PubChem, GTEx, ClinVar, PubMed and ClinicalTrials.gov under each provider’s terms. KinomeX records preserve source identifiers and links. ChEMBL-derived redistribution remains subject to CC BY-SA 3.0; UniProt, STRING and AlphaFold DB data are CC BY 4.0; RCSB PDB data are provided under CC0. PubMed abstract copyright is publication specific. KinHub roster redistribution should be reviewed with its maintainers because an explicit current data licence was not identified. A complete attribution and reuse notice is included with the software.")
    ...
    items = [
        ...
        "Rerun the catalogue, database and ligand-coverage audits for the final archived snapshot.",
